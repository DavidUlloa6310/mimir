...
func (h *TicketHandler) TicketsHandler(w http.ResponseWriter, r *http.Request) {
	...
	h.Cache.mu.RLock()
	cacheValid := time.Since(h.Cache.lastUpdateTime) < 5*time.Minute
	h.Cache.mu.RUnlock()
	...
	if cacheValid {
		...
		h.Cache.mu.RUnlock()
		log.Println("Using cached clustering results")
	} else {
		...
		clusters, err = database.TFIDFKMeansClusteringWithFallback(shortDescriptions)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		...
		h.Cache.mu.Unlock()
		log.Println("Updated clustering cache")
	}
